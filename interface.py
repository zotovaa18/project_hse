...
def video_to_brilliant(): #функция создания конспекта
    ...
    if not os.path.isfile(name_video):
        ...
        os.remove("t.txt")
        os.remove("t_new.txt")
        os.remove("daniil_text_end.txt")
        os.remove("timofey_text1.txt")
        os.remove("timofey_text2.txt")
        os.remove("timofey_text3.txt")
        os.remove("timofey_text4.txt")
        os.remove("timofey_text5.txt")
        os.remove("audio.wav")
...
def clicked_btn1(): #функции для создания перевода
    ...
    def clicked_ok():
        if not os.path.isfile(file_path.get()) or (re.search('.tex$', file_path.get())==None):
            ...
            messagebox.showinfo("Результат", "Программа завершилась")
