...
def pun():
    ...
    for i in range(len(nt)):
        path="burik"+str(i)+".txt"
        file=open(path,"w")
        file.write(nt[i])
        file.close()
    for k in range(i+1):
        path2="new_bur_"+str(k)+".txt"
        f=open(path2,"w")
        f.close()
    while m in range (i):
        new_file_path="new_bur_"+str(m)+".txt"
        sys.argv[2]=new_file_path
        file_path="burik"+str(m)+".txt"
        if open(file_path).read()!="":
            punct(file_path)
        m=m+1
    m=0
    while m<=i:
        file_path="new_bur_"+str(m)+".txt"
